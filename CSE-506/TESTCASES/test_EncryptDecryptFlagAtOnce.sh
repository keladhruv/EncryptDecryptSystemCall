#!/bin/sh
# 
# Test Encrypt Decrypt Functionality  with BOTH ENCRYPT DECRYPT OPTION GIVEN AT ONCE.
...
#Encrypt -e Decrypt -d given at once
./xhw1 "${TESTFILE}" "${OUTPUTFILE}" -e -d -p "${PASSWORD}" #-e and -d given
retval=$?
